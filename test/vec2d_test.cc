...
TEST(Vec2dTest, NomralCases) {
  Vec2d pt(2, 3);
  EXPECT_NEAR(pt.norm(), std::sqrt(13.0), 1e-5);
  EXPECT_NEAR(pt.squaredNorm(), 13.0, 1e-5);
  EXPECT_NEAR(pt.distanceTo(Vec2d(0,0)), std::sqrt(13.0), 1e-5);

  EXPECT_NEAR(pt.distanceTo({0, 2}), std::sqrt(5.0), 1e-5);

//  EXPECT_NEAR(pt.Angle(), std::atan2(3, 2), 1e-5);
  EXPECT_NEAR(pt.dot(Vec2d(4, 5)), 23, 1e-5);
  EXPECT_EQ(pt.DebugString(), "vec2d ( x = 2 , y = 3 )");
  pt.setX(4);
  pt.setY(5);
  EXPECT_NEAR(pt.norm(), std::sqrt(41.0), 1e-5);
  EXPECT_NEAR(pt.squaredNorm(), 41.0, 1e-5);
  pt.normalize();
  EXPECT_NEAR(pt.x(), 4.0 / std::sqrt(41.0), 1e-5);
  EXPECT_NEAR(pt.y(), 5.0 / std::sqrt(41.0), 1e-5);
  EXPECT_NEAR(pt.norm(), 1.0, 1e-5);
